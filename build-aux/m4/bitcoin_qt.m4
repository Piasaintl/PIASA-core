...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; piasa-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build piasa-qt GUI (default=auto, qt5 tried first)])],
    [
...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  BITCOIN_QT_CHECK([
  TPIASA_CPPFLAGS=$CPPFLAGS
  TPIASA_CXXFLAGS=$CXXFLAGS
  CPPFLAGS="$QT_INCLUDES $CPPFLAGS"
  CXXFLAGS="$PIC_FLAGS $CXXFLAGS"
  if test x$bitcoin_qt_got_major_vers = x5; then
    TPIASA_CPPFLAGS="$TPIASA_CPPFLAGS -DHAVE_QT5"
  ...
  CPPFLAGS=$TPIASA_CPPFLAGS
  CXXFLAGS=$TPIASA_CXXFLAGS
  ])
...
AC_DEFUN([_BITCOIN_QT_CHECK_STATIC_PLUGINS],[
  ...
  CHECK_STATIC_PLUGINS_TPIASA_LIBS="$LIBS"
  LIBS="$2 $QT_LIBS $LIBS"
  ...
  LIBS="$CHECK_STATIC_PLUGINS_TPIASA_LIBS"
])
