...
#include "ui_zpiasacontroldialog.h"
...
std::list<std::string> ZPIASAControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZPIASAControlDialog::listMints;

ZPIASAControlDialog::ZPIASAControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZPIASAControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZPIASAControlDialog::~ZPIASAControlDialog()
{
    delete ui;
}

void ZPIASAControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPIASAControlDialog::updateList()
{
...
void ZPIASAControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPIASAControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZPIASA_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPIASAControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZPIASAControlDialog::GetSelectedMints()
{
...
void ZPIASAControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
