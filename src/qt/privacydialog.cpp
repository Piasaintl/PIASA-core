                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zPIASA ought to be enough for anybody." - Bill Gates, 2017
    ui->zPIASApayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzPIASASyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzPIASA->setEnabled(false);
        ui->pushButtonMintzPIASA->setToolTip(tr("zPIASA is currently disabled due to maintenance."));

        ui->pushButtonSpendzPIASA->setEnabled(false);
        ui->pushButtonSpendzPIASA->setToolTip(tr("zPIASA is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zPIASApayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzPIASA_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPIASA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zPIASA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zPIASA in ") +
...
void PrivacyDialog::on_pushButtonSpendzPIASA_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPIASA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zPIASA
        sendzPIASA();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPIASA
    sendzPIASA();
}

void PrivacyDialog::on_pushButtonZPIASAControl_clicked()
{
    ZPIASAControlDialog* zPIASAControl = new ZPIASAControlDialog(this);
    zPIASAControl->setModel(walletModel);
    zPIASAControl->exec();
}

void PrivacyDialog::setZPIASAControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzPIASASelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzPIASA()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid PIASA Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zPIASApayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zPIASA is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PIASA </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zPIASA</b>";
    ...
    // use mints from zPIASA selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZPIASAControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZPIASAControlDialog::GetSelectedMints();
    }
    ...
    // Clear zpiasa selector in case it was used
    ZPIASAControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zPIASA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " PIASA, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zPIASA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPIASA </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zPIASA "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zPIASA "));
    ui->labelzPIASAAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzPIASASyncStatus->setVisible(fShow);
}
