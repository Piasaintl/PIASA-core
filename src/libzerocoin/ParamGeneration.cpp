...
CalculateParams(ZerocoinParams &params, CBigNum N, string aux, uint32_t securityLevel)
{
	...
	for (uint32_t count = 0; count < MAX_ACCUMGEN_ATTPIASATS && params.accumulatorParams.accumulatorBase.isOne(); count++) {
...
deriveIntegerGroupFromOrder(CBigNum &groupOrder)
{
	...
	for (uint32_t i = 1; i < NUM_SCHNORRGEN_ATTPIASATS; i++) {
...
calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, CBigNum modulus, CBigNum groupOrder, uint32_t index)
{
	...
	for (uint32_t count = 1; count < MAX_GENERATOR_ATTPIASATS; count++) {
                    ...
                    uint32_t *prime_gen_counter)
{
	...
	// If primeBitLen >= 33 bits, perform the recursive case.
		...
		for (uint32_t testNum = 0; testNum < MAX_PRIMEGEN_ATTPIASATS; testNum++) {
	...
	// We only reach this point if the test loop has iterated MAX_PRIMEGEN_ATTPIASATS
